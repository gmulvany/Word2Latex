...
def generate_tex(input_file):
	...
	if not os.path.exists(doc_file):
		...
		try:
				...
				print(bold_red,  "Error: Attempted but failed to convert document:", e, reset)
	return False
...
def manual_fixes(input_file):
	...
	try: #Get Abstract
		...
		if abstract_texts.__len__() == 1:
			abstract_text = pypandoc.convert_file(abstract_texts[0], to_format, input_format, extra_args=extra_args)
	...
	replace_line_with_pattern(tex_file, "\\acmYear{", r"\acmYear{" + current_year.__str__() + "}")
	replace_line_with_pattern(tex_file, "copyrightyear{", "\\copyrightyear{" + current_year.__str__() + "}")
	...
	for author in reversed(all_author_info):
		if author['postcode'].__str__() !='nan':
			add_line_below_pattern(tex_file, r"of authors' names for this purpose.", r"    \postcode{" + author['postcode'].__str__() + "}")
		if author['country'].__str__() !='nan':
			add_line_below_pattern(tex_file, r"of authors' names for this purpose.", r"    \country{" + author['country'].__str__() + "}")
		if author['state'].__str__() !='nan':
			add_line_below_pattern(tex_file, r"of authors' names for this purpose.", r"    \state{" + author['state'].__str__() + "}")
		if author['city'].__str__() !='nan':
			add_line_below_pattern(tex_file, r"of authors' names for this purpose.", r"    \city{" + author['city'].__str__() + "}")
		if author['street'].__str__() !='nan':
			add_line_below_pattern(tex_file, r"of authors' names for this purpose.", r"    \streetaddress{" + author['street'].__str__() + "}")
		if author['institution'] !='nan':
			add_line_below_pattern(tex_file, r"of authors' names for this purpose.", r"    \institution{" + author['institution'].__str__() + "}")
		add_line_below_pattern(tex_file, r"of authors' names for this purpose.", r"\affiliation{%")
		if author['email'].__str__() !='nan':
			add_line_below_pattern(tex_file, r"of authors' names for this purpose.", r"\email{" + author['email'].__str__() + "}")
		if author['mark'].__str__() !='nan':
			add_line_below_pattern(tex_file, r"of authors' names for this purpose.", r"\authornotemark{" + author['mark'].__str__() + "}")
		if author['orcid'].__str__() !='nan':
			add_line_below_pattern(tex_file, r"of authors' names for this purpose.", r"\orcid{" + author['orcid'].__str__() + "}")
		if author['note'].__str__() !='nan':
			add_line_below_pattern(tex_file, r"of authors' names for this purpose.", r"\authornote{" + author['note'].__str__() + "}")
		if author['name'].__str__() !='nan':
			add_line_below_pattern(tex_file, r"of authors' names for this purpose.", r"\author{" + author['name'].__str__() + "}")
...
def make_acm_figure(name, i, tex_file, subdoc_captions, description):
	...
	try:#Make figures text width and add image filepath
		replace_line_with_pattern(tex_file, r'^\\includegraphics\[width=.*in,height=.*in\]{', replacement)
	except Exception as e:
	...
	saved_note = replace_line_with_pattern(tex_file, search_pattern, "") #Replace the original under-image caption with only the "after-note" section
			...
			if "image" in doc.part.rels[rel].target_ref:
				...
				try:
					...
					if pic_elem is not None:
						description = root.find(".//pic:pic/pic:nvPicPr/pic:cNvPr", namespaces=namespaces).get('descr')
						image_alts.append(description)
					search = ".//a:blip[@r:embed='{}']".format(next_rId)
					pic_elem = root.find(search, namespaces=namespaces)
					if pic_elem is not None:
						description = root.find(".//pic:pic/pic:nvPicPr/pic:cNvPr", namespaces=namespaces).get('descr')
						image_alts.append(description)
...
def handle_failed_chapter(e):
	print(bold_red, "Failed to convert file", " Error:", e, reset)
